...
def validate_jubilee_manager_config(manager: JubileeManager) -> bool:
    ...
    if manager.deck is None:
        ...
        print(
            f"  ✓ Deck initialized: {manager.deck.name if hasattr(manager.deck, 'name') else 'unnamed'}"
        )
    ...
    if manager.deck and hasattr(manager.deck, "slots"):
        ...
        if invalid_wells:
            warnings.append(
                f"⚠ Wells not accessible or wrong type: {', '.join(invalid_wells)}"
            )
            print(f"  ⚠ Invalid wells: {', '.join(invalid_wells)}")

        if well_property_issues:
            warnings.append(
                f"⚠ Wells with missing properties: {len(well_property_issues)}"
            )
    ...
    if manager.scale is None:
        ...
        scale_coords_set = (
            hasattr(manager.scale, "x")
            and hasattr(manager.scale, "y")
            and hasattr(manager.scale, "z")
        )
        ...
        if scale_coords_set:
            ...
            if (
                manager.scale.x is ...
                or manager.scale.y is ...
                or manager.scale.z is ...
            ):
                errors.append("❌ CRITICAL: Scale coordinates not set (still Ellipsis)")
                validation_passed = False
                print("  ❌ Scale coordinates not configured:")
                print(f"     X: {manager.scale.x}")
                print(f"     Y: {manager.scale.y}")
                print(f"     Z: {manager.scale.z}")
                print(
                    "  ⚠ You must set scale.x, scale.y, scale.z in JubileeManager.__init__"
                )
            else:
                print("  ✓ Scale coordinates set:")
...
def main():
    ...
    well_input = input(
        f"Enter well ID to test (0-17, default: {TEST_WELL_ID}): "
    ).strip()
    if well_input:
        TEST_WELL_ID = well_input

    weight_input = input(
        f"Enter target weight in grams (default: {TARGET_WEIGHT}): "
    ).strip()
    if weight_input:
        try:
    ...
    feedrate_input = (
        input(f"Enter feedrate (fast/medium/slow, default: {FEEDRATE.value}): ")
        .strip()
        .lower()
    )
    if feedrate_input:
    ...
    print(f"  Feedrate: {FEEDRATE.value} ({FEEDRATE.value} mm/min)")
    ...
    try:
        ...
        wait_for_user(
            "Ready to connect to Jubilee and scale. Press Enter to continue..."
        )
        ...
        print("  - The mold does NOT have a top piston")
        print("  - The area is clear for movement")
        ...
        if abs(tared_weight) > 0.01:
            print(
                f"⚠ Warning: Scale reading after tare is {tared_weight:.4f}g (expected ~0.00g)"
            )
        ...
        wait_for_user(
            f"Add approximately {TARGET_WEIGHT}g of material to the mold, then press Enter..."
        )
        ...
        for i in range(5):
            ...
            print(
                f"  Reading {i + 1}/5: {current_weight:.4f}g (target: {TARGET_WEIGHT:.4f}g)"
            )
            time.sleep(1)
        ...
        if abs(final_weight - TARGET_WEIGHT) < 0.1:
            print("✓ Weight is within tolerance (±0.1g)")
        else:
            ...
            try:
                ...
                if manager.manipulator and manager.manipulator.is_carrying_well():
                    ...
                    try:
                        ...
                        print(
                            "Please manually return the mold to its original location"
                        )
