...
def get_mold_selection() -> str:
    ...
    while True:
        try:
            user_input = input("\nEnter mold number to dispense (0-17) or 'q' to quit: ").strip()
...
def get_target_weight() -> float:
    ...
    while True:
        try:
            user_input = input(f"Enter target weight in grams (default: {DEFAULT_WEIGHT}): ").strip()
...
def dispense_with_tamp(manager: JubileeManager, well_id: str, target_weight: float) -> bool:
    """
...
def main():
    ...
    try:
        ...
        num_dispensers_input = input(f"Enter number of piston dispensers (default: {NUM_DISPENSERS}): ").strip()
        num_dispensers = int(num_dispensers_input) if num_dispensers_input else NUM_DISPENSERS
        
        pistons_input = input(f"Enter pistons per dispenser (default: {PISTONS_PER_DISPENSER}): ").strip()
        pistons_per_dispenser = int(pistons_input) if pistons_input else PISTONS_PER_DISPENSER
        ...
        while True:
            ...
            # Confirm operation
            confirm = input(f"\nDispense {target_weight}g to mold {mold_id}? (y/n): ").strip().lower()
