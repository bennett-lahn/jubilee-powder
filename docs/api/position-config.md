...
| Key | Purpose |
|-----|---------|
| `z_heights` | Named Z levels (e.g., `"mold_transfer_safe"`). Positions whose `z` is `"USE_Z_HEIGHT_POLICY"` resolve their Z from the context's active z-height at runtime. |
...
| `actions` | Tool operations (pick mold, retrieve piston, etc.). Validated and executed without changing the current position. |
...
???+ note "Engagement requirements fallback"
...
4. **`scale_active` is only entered via `place_mold_on_scale`, not via a normal move.**
    ...
    !!! danger "Never add scale_active to allowed_destinations"
        The state machine sets `context.position_id = "scale_active"` and `context.mold_on_scale = True` together after the place action succeeds. Adding `scale_active` to the `allowed_destinations` of any position would allow the machine to navigate to it with an incorrect procedure, probably causing a collision.
...
- [Configuration Guide](../how-to/configuration.md) - editing workflow for both config files
- [ConfigLoader](config-loader.md) - `system_config.json` (tool names referenced by position requirements)
- [MotionPlatformStateMachine](motion-platform.md) - runtime validation and state tracking
- [JubileeManager](jubilee-manager.md) - high-level interface that loads this file at `connect()`
