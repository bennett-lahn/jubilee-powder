...
def main():
    ...
    try:
        ...
        manipulator = Manipulator(
            index=0, name="manipulator", state_machine=state_machine
        )
        ...
        print(
            f"  X={current_pos['X']}, Y={current_pos['Y']}, "
        ...
        if state_machine.context.payload_state != "mold_without_top_piston":
            print_error(
                f"Payload state is '{state_machine.context.payload_state}', must be 'mold_without_top_piston'"
            )
            prerequisites_met = False
        else:
        ...
        if (
            state_machine.context.current_well
            and state_machine.context.current_well.has_top_piston
        ):
        ...
        if (
            not state_machine.context.piston_dispensers
            or len(state_machine.context.piston_dispensers) == 0
        ):
            ...
            print_success(
                f"Dispenser 0 has {state_machine.context.piston_dispensers[0].num_pistons} pistons ✓"
            )

        # Check 7: Z-height is appropriate for dispenser operations
        if state_machine.context.z_height_id != "mold_transfer_safe":
            print_error(
                f"Z-height '{state_machine.context.z_height_id}' may not be suitable for dispenser operations"
            )
            prerequisites_met = False
        else:
        ...
        if not state_machine._registry.has(piston_dispenser_0.ready_pos):
            print_error(
                f"Dispenser position '{piston_dispenser_0.ready_pos}' not found in registry"
            )
            prerequisites_met = False
        else:
            ...
            if not dispenser_pos.coordinates:
                print_error(
                    f"Dispenser position '{piston_dispenser_0.ready_pos}' has no coordinates"
                )
                prerequisites_met = False
            else:
                print_success(
                    f"Dispenser position '{piston_dispenser_0.ready_pos}' configured ✓"
                )
                print(
                    f"  Coordinates: x={dispenser_pos.coordinates.x}, y={dispenser_pos.coordinates.y}"
                )
        ...
        while continue_retrieval:
            ...
            print_step(
                piston_count,
                f"Piston {piston_count} - Move to dispenser 0 ready position",
            )
            ...
            current_pos = machine.get_position()
            print(
                f"Position: X={current_pos['X']}, Y={current_pos['Y']}, "
                f"Z={current_pos['Z']}, V={current_pos.get('V', 'N/A')}"
            )
            ...
            print_step(
                piston_count,
                f"Piston {piston_count} - Retrieve piston from dispenser 0",
            )

            print(
                f"Dispenser 0 pistons before retrieval: {piston_dispenser_0.num_pistons}"
            )
            print(f"Mold has piston before retrieval: {mock_mold.has_top_piston}")

            wait_for_user_confirmation(
                "Retrieve piston from dispenser 0 (manipulator will move)"
            )
            ...
            print(
                f"Dispenser 0 pistons after retrieval: {piston_dispenser_0.num_pistons}"
            )
        ...
        print(f"  Payload state: {state_machine.context.payload_state}")
        print(
            f"  Carrying mold: {state_machine.context.current_well.name if state_machine.context.current_well else 'None'}"
        )
        if state_machine.context.current_well:
            print(
                f"  Mold has piston: {state_machine.context.current_well.has_top_piston}"
            )
