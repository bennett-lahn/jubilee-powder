...
def main():
    ...
    try:
        ...
        scale.connect()
        print_success("Connected to scale")
        ...
        state_machine = MotionPlatformStateMachine.from_config_file(
            config_path, machine, scale=scale, feedrate=FeedRate.MEDIUM
        )
        ...
        print(
            f"Current position: X={current_pos['X']}, Y={current_pos['Y']}, "
        ...
        wait_for_user_confirmation(
            f"Fill mold with {TARGET_WEIGHT} grams of powder (trickler will move)"
        )
        ...
        wait_for_user_confirmation(
            f"Place mold {MOLD_ID} back (manipulator will move down)"
        )
        ...
        print("  Final position: global_ready")
        print(f"  Payload state: {state_machine.context.payload_state}")
        if state_machine.context.current_well:
            print(f"  Carrying: {state_machine.context.current_well.name}")
            print(
                f"  Has top piston: {state_machine.context.current_well.has_top_piston}"
            )
